function CalulaPrestacao(valorFinanciado, t, p) {
  let prestacao = (valorFinanciado * t) / (1 - Math.pow(1 + t, -p));
  return prestacao;
}

function CalculaCoefDeFinanciamento(t, p) {
  var CF = (t * Math.pow(1 + t, p)) / (Math.pow(1 + t, p) - 1);
  return CF;
}

function CalculaValorPago(valorFinanciado, t, p) {
  let prestacao = CalulaPrestacao(valorFinanciado, t, p);
  var valorPago = prestacao * p;
  return valorPago;
}

function AproximaTaxaReal(valorFinanciado, p, valorPago) {
  const precisao = 0.000001; // Precisao do método de newton
  var estimativa = 0.1; // Estimativa inicial
  var iteracoes = 0; 
  ...
  for (; Math.abs(erro) > precisao; ) {
    ...
    if (iteracoes > 1000) {
      // Garante a convergência do método
...
function CalculaValorJaCorrigido(valorPago, t, p) {
...
document.getElementById("submitButton").onclick = function (e) {
  ...
  var prestacao = CalulaPrestacao(valorFinanciado, t, p);
  var CF = CalculaCoefDeFinanciamento(t, p);
  var valorPago = CalculaValorPago(valorFinanciado, t, p);
  var treal = AproximaTaxaReal(valorFinanciado, p, valorPago);
  var t_anual = 1 + (Math.pow(1 + t, 12) - 1) - 1;
  var valorCorrigido = CalculaValorJaCorrigido(valorPago, t, p);
